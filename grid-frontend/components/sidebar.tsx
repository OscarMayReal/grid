...
import { ActivityIcon, ArchiveIcon, ArrowUpCircleIcon, GlobeIcon, LaptopMinimalIcon, PlusIcon, ShieldIcon, ShoppingBagIcon, UsersIcon } from "lucide-react";
...
export const AdminSidebar = ({ignoreSize}: {ignoreSize?: boolean}) => {
    ...
    return (
        <div className="sidebar">
            ...
            <SidebarItem title="All Devices" onClick={() => {router.push("/admin/resources")}} Icon={LaptopMinimalIcon} active={path === "/admin/resources"} index={1} />
            <SidebarItem title="Store" onClick={() => {router.push("/admin/assignments")}} Icon={ShoppingBagIcon} active={path === "/admin/assignments"} index={2} />
            <SidebarItem title="Settings" onClick={() => {router.push("/admin/settings")}} Icon={SettingsIcon} active={path === "/admin/settings"} index={3} />
            <SidebarItem title="Updates" onClick={() => {router.push("/admin/desktops")}} Icon={ArrowUpCircleIcon} active={path === "/admin/desktops"} index={4} /> 
            <SidebarItem title="Monitoring" onClick={() => {router.push("/admin/monitoring")}} Icon={ActivityIcon} active={path === "/admin/monitoring"} index={5} />
            <SidebarItem title="Users" onClick={() => {router.push("/admin/users")}} Icon={UsersIcon} active={path === "/admin/users"} index={6} />
            <Separator style={{margin: "10px 0px"}} />
            <SidebarItem title="Access" onClick={() => {router.push("/access")}} Icon={GlobeIcon} active={false} index={7} />
        </div>
    );
};
